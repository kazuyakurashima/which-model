...
# （plugins/which-model/skills/pick/references/ai-model-guides/）へ同期する。
#
...
DEST="${REPO_DIR}/plugins/which-model/skills/pick/references/ai-model-guides"
...
if [[ "${1:-}" == "--check" ]]; then
  if diff -rq "$SRC" "$DEST" >/dev/null 2>&1; then
    ...
    echo "ズレ検出：docs/ai-model-guides/ と plugins/which-model/skills/pick/references/ai-model-guides/ が一致しません。" >&2
...
cp -R "$SRC" "$DEST"
echo "同期しました: docs/ai-model-guides/ → plugins/which-model/skills/pick/references/ai-model-guides/"
