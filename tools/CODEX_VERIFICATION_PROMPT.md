...
- `00_index.md` … 全体の使い方
- `01_sources_evidence.md` … **根拠台帳**。全 `[Official]` 記述の source_id（S1〜S143。欠番：S4, S5, S52, S53。Retired：S1, S2, S3, S7, S49）と公式出典（P1〜P28。欠番：P10）の対応表。ここが正の起点。
...
### 公式一次情報（`01` の P1〜P28。P10 は欠番・未参照。ここだけを事実の根拠にする）
...
- P27 Covered Models（Anthropic 公式サポート記事） — https://support.claude.com/en/articles/15425695-covered-models
- P28 Pricing（Anthropic 公式ドキュメント） — https://platform.claude.com/docs/en/about-claude/pricing
...
1. **`[Official]` 記述の事実性。** `01` の各 source_id（S1〜S143。欠番：S4, S5, S52, S53。Retired：S1, S2, S3, S7, S49）の主張が、記載された出典ページ
   （P1〜P28。P10 は欠番）の内容と一致するかを確認する。古い・誤り・過大/過小表現があれば指摘する。
...
- **S-id の参照整合性**：`00,02,03,04,05` の各ファイルが参照する `S<数字>` が、すべて `01` に
  定義されているか。※この観点はリポジトリの `python3 tools/check-ledger-consistency.py` で
  機械検査できる（ダングリング参照・Retired 参照・「8. 出典」欄の一致・モデル別ガイドが引く
  S-id の「対象」欄の整合）。あなたは**その検査が拾えない意味的な不整合**に注力してよい。定義のない参照（ダングリング）、および定義済みだがどこからも参照されない
...
- **モデル ID・数値の内部整合**：**`claude-fable-5-1`** / **`claude-opus-5`** / `claude-sonnet-5`、
  価格（$10/$50, $5/$25, **$2/$10**）、キャッシュ読み $0.25/MTok（Fable 5.1 のみ 0.025 倍）、2576px、
  ~30% トークナイザ増（Sonnet 5）、64k、128k、512 トークン（キャッシュ最小長）、v2.1.219（Opus 5）、
  **v2.1.255（Fable 5.1）** などの数値がファイル間で食い違っていないか。
  **Sonnet 5 の価格は $2/$10 が正**（launch 時の導入価格が標準価格になり、2026-09-01 の $3/$15 への
  値上げは行われない — S15・P28）。**$3/$15 と書いてあれば古い**。
...
- **Covered Model / データ保持**。Covered Model（30 日保持必須・ZDR 不可）に指定されているのは
  ...
  **ZDR の例外は P27（Covered Models 記事）にだけ具体名がある**：Enterprise Frontier Safeguards（EFS）と、
  その移行措置として eligible customers に期間限定で Fable 5 / 5.1 の ZDR 利用を提供する旨。P20 は
  "unless expressly authorized by Anthropic" とだけ書く。**P20 だけを見て「例外は無い」と判定しないこと**。
...
- 事実判定は **P1〜P28（P10 は欠番）の実ページ内容のみ** を根拠にする。あなたの記憶や訓練データの一般知識で
  「こうだろう」と補完しない。
