...
などで切り替え、中身を確認して**もう一度 `y`** を送れば実行されます。普段の軽い作業では
呼ばず、設定済みのモデルでそのまま指示すれば十分です。
