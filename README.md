...
<details>
<summary>v3.9.0（旧 standalone 版）から移行する方は、必ずこちらを読んでください</summary>
...
**4.0.0 では、料理人もレシピ本もプラグインに同梱**されています（どちらもインストール時に一緒に入ります）。
プロジェクト側に置いたレシピ本は読みません（[カスタマイズについて](#カスタマイズについて)を参照）。
...
- **一度呼べば、そのセッション中は指示が効き続けます。** 2回目以降の依頼で毎回コマンドを打ち直す
  必要はありません（新しい依頼を書けば、フェーズ1からやり直します）。
...
**4.0.0 では、判断材料をプロジェクトごとに差し替えることはできません。** skill はプラグインに
同梱されたレシピ本だけを読みます（プロジェクト側に `docs/ai-model-guides/` を置いても読まれません）。

理由は、誰の環境でも同じ判断材料で動くようにするためです。プロジェクトごとの上書きを許すと、
クローンしてきたリポジトリに置かれたファイルを、それと知らずに判断材料として読んでしまう余地が
生まれます。利用者ゼロの段階でその境界を抱えるより、まず一本化することを選びました。
