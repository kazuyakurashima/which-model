...
**2. フェーズ1：推奨・代替の候補が選択UIで表示される**

![選択UI「Model」。質問文は「どのモデルで最適化しますか？」、続けて「判断材料：Last verified 2026-07-25（モデルの仕様・価格は陳腐化する。古ければ更新を）」「選択後に最適化プロンプトが表示されない場合は、次のメッセージで半角の p を送ってください。」。選択肢は 1. 推奨: Fable 5 / high（影響範囲の広い作り直しを、調査から計画・実装まで長く一貫して進めるのに向く。手戻りコストが大きい場面向き）、2. 代替: Opus 5 / xhigh（対話的に段階を刻んで少しずつ確認しながら進めたいときに選ぶ）、3. 中止（何もせず終了する）、4. Type something.（自由入力）、5. Chat about this。最下部に「Enter to select · ↑/↓ to navigate · Esc to cancel」](docs/images/demo-2-phase1.png)
...
2. skill が指示内容（設計・実装・リファクタ等）と複雑さを判定し、**推奨・代替の候補**を
   理由つきで選択UIに出す（**この時点ではまだモデルを切り替えない**）
   - 推奨 … 判断表の第一候補（通常の推奨開始点）
   - 代替 … 判断表の第二候補（別条件での代替）。**一律に安い・速い候補ではなく、行によっては
     推奨より高能力・高価格**です。選ぶ状況は判断表に明記がある行だけ表示されます
...
選択UIが出たら、モデルはまだ切り替えずに候補を選びます（推奨／代替／中止）。すると今の
...
- **合図は `p`（表示）と `g`（実行）の2つです**（6.0.0 の破壊的変更）。5.1.0 までは `y` が実行の
  ...
  **設定を忘れて `g` を送ると現在の設定のまま実行されます**。プロンプトを編集して送る経路も
  使えます。
- **提示の保証範囲は、判断表の第一候補・第二候補までです。** 提示後に「コスト度外視なら」
  「成果優先なら」など、**判断表にない別基準で再選定する動作は未定義**で、結果の一貫性を
  保証しません（判断表にその軸の列が無いため、回答はその場の判断になります）。候補の意味を
  目的別（通常推奨・成果優先・効率優先）へ整理する作業は次版の課題です。
...
- **対象は対話型の Claude Code です**（VS Code 拡張・対話ターミナルのどちらでも動きます）。
  ...
  提示する経路に自動で切り替わります（確定合図は `p` / `p opus` / `p fable` / `p sonnet` / `n`。
