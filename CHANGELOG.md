...
8.0.0 のレビューで見つかった食い違いの修正。**判断表・選択UI・合図の文法は変更なし。**

### Fixed

- **受入記録に残っていた原因の断定を外した。** 8e の記録が「バージョン不足ではなく gateway 側が
  Fable 5.1 に未対応」と書いており、「原因は未特定」とする README・リリースノートと不一致だった。
  **「この実施環境では一覧に表示されず確認できなかった。原因は未特定」**に統一し、観測・
  `[Official]`（S142）・原因・言えないことを分けて表に整理した
- **Sonnet 5 の価格訂正の追随漏れ。** $2/$10 への訂正を入れたのに、**S12 と `05` に「Sonnet 4.6 と
  同価格」が残っていた**（4.6 は $3/$15 なので成り立たない）。同価格の主張を落とし、価格は S15 を
  正とすることを明記して同梱コピーへ同期した
- **`SKILL.md` の追記文が自らの禁止語に触れていた。** 手順の禁止語リストは利用者向け表示での
  `代替` を禁じているのに、追加した1文が「利用可能な代替モデル」と書いていた。
  **「利用可能な別のモデル」**へ置換した
- **同じ1文の「README の注記」を該当箇所への直リンクにした。** 配布物の README はスタブで
  この注記を持たないため、参照先が辿れなかった

### Changed

- 受入記録の表が説明の挿入で分断されていたのを、説明を表の外へ移して解消した
- 8b の結果表記を実態に合わせた：**表示は対話環境、ガイド読み込みは headless で確認**（2 経路に
  分かれた確認であり、1 経路で両方を満たしたわけではない）
